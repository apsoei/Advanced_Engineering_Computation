...

{
	...
	for(auto vtHd : plgList.back().vtHd)
	{
		auto searchKey=vtHd.vtxPtr->searchKey;
		auto found=vtxToPlg.find(searchKey);
		...
		{
			std::vector <PolygonHandle> newTable;
			newTable.push_back(plHd);
			vtxToPlg[searchKey]=newTable;
		}
	}
...
void PolygonalMesh::SetPolygonVertex(PolygonHandle plHd,int nPlVt,const VertexHandle plVtHd[])
{
	...
	for(int i=0; i<nPlVt; ++i)
	{
		...
		vtxToPlg[GetSearchKey(plVtHd[i])].push_back(plHd);
	}
}
