...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		for(int y = thisY0; y<(thisY0 + hei); ++y)
		{
			for(int x= thisX0; x<(thisX0 + wid);++x)
			{
				if(true == this->IsInRange(x,y))
				{
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))] = this->bmpPtr[4*(y*this->GetWidth() + x)];
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))+1] = this->bmpPtr[4*(y*this->GetWidth() + x)+1];
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))+2] = this->bmpPtr[4*(y*this->GetWidth() + x)+2];
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))+3] = this->bmpPtr[4*(y*this->GetWidth() + x)+3];

					// destination.bmpPtr[(y-thisY0)*wid +(x-thisX0)] = 4;
					// if (false ==testPrint) 
					// {
				...
				{
					// std::cout << "this statement should not be printed!! " <<std::endl;
					// destination.bmpPtr[(y-thisY0)*wid +(x-thisX0)]=clearColor;
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))] = clearColor;
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))+1] = clearColor;
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))+2] = clearColor;
					destination.bmpPtr[4*((y-thisY0)*wid +(x-thisX0))+3] = clearColor;
				}	
			}	
		}
