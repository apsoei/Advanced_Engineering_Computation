...
protected:
	...
	void autoResize_base(TableClass &table)
	{
		...
		unsigned int numElems = this->len;
		// unsigned int numElems = table.len;
// std::cout << "table.size() : "<< table.size() << std::endl;
// std::cout << "this->len : "<< this->len << std::endl;
// std::cout << "this->size : "<< this->size() << std::endl;
		int newNumRows = 0;
		// double ratio = numRows/numElems;
		
		if(numElems>k1*numRows)
		{
// std::cout<< "auto resize ( GROW ) TRIGGERED" << std::endl;
			newNumRows = 2*numRows;
			resize_base(table,newNumRows);
		}
		else if(numElems<numRows/k2)
		{
// std::cout<< "auto resize ( SHRINK ) TRIGGERED" << std::endl;
	...
	// Keep track of the number of elements in the table.  
	// (The number must be updated in which functions?)
	// If the number of elements exceeds k times the length of the table (number of rows), 
	// grow the table.
