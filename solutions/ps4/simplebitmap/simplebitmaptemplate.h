	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		for(int dx=0; dx<wid; ++dx)
		{
			...
			{
				auto dstPixPtr=destination.GetEditablePixelPointer(dx,dy);
				if(true==this->IsInRange(thisX0+dx,thisY0+dy))
				{
					auto srcPixPtr=this->GetPixelPointer(thisX0+dx,thisY0+dy);
					for(int i=0; i<NumComponentPerPixel; ++i)
					{
						dstPixPtr[i]=srcPixPtr[i];
					}
				}
				else
				{
